...
namespace Soulstone.Duality.Plugins.Cupboard.Components.UI
{
    ...
    {
        // TODO: The logic for displaying and keeping the IP should be moved
        // to the GameContext at some point.
        private static string _ipText = null;
        ...
        {
            if (!(DualityApp.UserData.CustomData is UserData userData))
            {
                Logs.Game.WriteError("Failed to retrieve user data.");
                return;
            }
            
            string endpoint = userData.ExternalIPEndpoint;
            if (string.IsNullOrEmpty(endpoint))
            {
                Logs.Game.WriteWarning("No external IP endpoint specified in user data.");
                return;
            }
            ...
            try
            {
                ...
                string ip = client.DownloadString(endpoint)?.Trim();
